...
import { BarChart3, PieChart, Settings, HelpCircle, LogOut, Home, TrendingUp, Search, ChevronDown } from "lucide-react"
    ...
    >
      ...
      <div className="flex flex-col items-center border-b py-4">
        <div className="relative h-12 w-12 overflow-hidden rounded-full">
          <Image src="/placeholder.svg?height=100&width=100" alt="User profile" fill className="object-cover" />
        </div>
        {isOpen && (
      ...
      <div className="flex-1 overflow-auto py-4">
        <nav className="space-y-1 px-2">
          <div className="mb-4">
            {isOpen && <h3 className="mb-2 px-4 text-xs font-semibold uppercase text-muted-foreground">Main Menu</h3>}
            <ul className="space-y-1">
              {mainNavItems.map((item) => (
                <li key={item.title}>
                  ...
                  >
                    <item.icon className={cn("h-5 w-5", !isOpen ? "mr-0" : "mr-2")} />
          ...
          <div className="mb-4">
            {isOpen && <h3 className="mb-2 px-4 text-xs font-semibold uppercase text-muted-foreground">Other</h3>}
            <ul className="space-y-1">
